'use client'; 
// 👆 Directive: Next.js Client Component because it contains interactive input fields for the profit simulator and clear history buttons.

import React, { useState } from 'react'; 
// 👆 Imports React and useState to track the user's custom trade size ($10,000) in the profit calculator.

import { ArbitrageOpportunity, ConsolidatedBook } from '@/types/market'; 
// 👆 Imports TypeScript interfaces for ArbitrageOpportunity and ConsolidatedBook.

import { ArrowRight, Calculator, Flame, TrendingUp } from 'lucide-react'; 
// 👆 Icons:
// - Flame: Glowing fire icon for hot arbitrage alerts
// - Calculator: Icon for the profit simulator widget
// - ArrowRight: Visual route indicator (e.g. Buy Binance ➔ Sell Kraken)
// - TrendingUp: Empty-state scanning radar icon

interface ArbitrageRadarProps {
  opportunities: ArbitrageOpportunity[]; // Array of the latest 50 detected opportunities from WebSocket
  book: ConsolidatedBook | null;          // Active order book
  onClearHistory: () => void;             // Callback function to clear the table
}
/**
 * HELPER 1: renderExchangeTag
 * Returns color-coded text for exchange names.
 */
...
/**
 * HELPER 2: formatMoney
 * Formats numbers into 2 decimal places with commas (e.g. 10,000.00).
 */
...
export default function ArbitrageRadar({
  ...
  // 1. STATE: User's simulated trade size in USD (defaults to $10,000)
  const [simCapital, setSimCapital] = useState<number>(10000);

  // 2. LATEST OPPORTUNITY: The newest opportunity in the stream (top of array)
    ...
    <div className="bg-[#0B0E14] border border-gray-800/80 rounded-xl overflow-hidden flex flex-col font-mono shadow-xl">
      ...
      <div className="px-4 py-3 border-b border-gray-800/80 flex items-center justify-between bg-[#0D111A]">
        <div className="flex items-center gap-2">
          ...
          {/* Badge showing count of detected opportunities */}
      ...
      {/* 2. LIVE PROFIT SIMULATOR WIDGET */}
      ...
      {/* 3. REAL-TIME ARBITRAGE OPPORTUNITIES FEED LIST */}
        ...
        {opportunities.length === 0 ? (
          ...
          opportunities.map((opp, idx) => {
            ...
            return (
              ...
              >
                {/* Row Header: Symbol, Execution Path & Profit Badge */}
